...
#define GTK_LIQUIDDRIVE_KNOB_GET_PRIVATE(obj) (G_TYPE_INSTANCE_GET_PRIVATE((obj), GTK_TYPE_LIQUIDDRIVE_KNOB, GtkLiquidDriveKnobPrivate))

G_DEFINE_TYPE(GtkLiquidDriveKnob, gtk_knob, GTK_TYPE_RANGE);

typedef struct _GtkLiquidDriveKnobPrivate GtkLiquidDriveKnobPrivate;

struct _GtkLiquidDriveKnobPrivate
{
    int knob_x;
    int knob_y;
    int knob_step;
...
static void knob_expose(GtkWidget *widget, int knob_x, int knob_y, GdkEventExpose *event, int arc_offset)
{
    /** check resize **/
    int grow;
    GtkLiquidDriveKnobPrivate *priv = GTK_LIQUIDDRIVE_KNOB_GET_PRIVATE(widget);
    ...
    if (priv->model != 3) {
        ...
        GtkLiquidDriveKnob *knob = GTK_LIQUIDDRIVE_KNOB(widget);
...
{
    g_assert(GTK_IS_LIQUIDDRIVE_KNOB(widget));
    GtkLiquidDriveKnob *knob = GTK_LIQUIDDRIVE_KNOB(widget);
    GtkLiquidDriveKnobPrivate *priv = GTK_LIQUIDDRIVE_KNOB_GET_PRIVATE(knob);
...
{
    GtkLiquidDriveKnob *knob = GTK_LIQUIDDRIVE_KNOB(object);
...
/****************************************************************
 ** init the GtkLiquidDriveKnobClass
 */

static void gtk_knob_class_init (GtkLiquidDriveKnobClass *klass)
{
...
//--------- connect the events with funktions
    ...
    g_type_class_add_private(obj_class, sizeof (GtkLiquidDriveKnobPrivate));
}
...
static void gtk_knob_init (GtkLiquidDriveKnob *knob)
{
...
{
    GtkWidget *widget = GTK_WIDGET( g_object_new (GTK_TYPE_LIQUIDDRIVE_KNOB, NULL ));
